...
export default function transpile(
    files: Map<string, { content: Buffer; type: string }>,
    entry: string
) {
    ...
    const getSourceFile = (filename: string) => {
        return createSourceFile(
            filename,
            files.get(filename)?.content.toString() ?? "",
            ScriptTarget.ES5
        );
    };
    ...
    const compilerHost: CompilerHost = {
        ...
        readFile: (fileName) => files.get(fileName)?.content.toString() ?? "",
